...
// TODO: Deprecate this once onMap is no longer used in gather-game-client
...
export class GameMapV2 extends GameMapBase {
  // @ts-expect-error Error auto-ignored when enabling TS strict. It's possible this is incorrect.
  // TODO: @ENG-4193 Clean these up! If you're already touching this code, please clean this up while you're at it.
...
export function convertGameMapV2ToGameMap(mapV2: GameMapV2): GameMap {
  const objArray = Object.keys(mapV2.objects ?? {})
    .sort((key1, key2) => parseInt(key1) - parseInt(key2))
    // @ts-expect-error Error auto-ignored when enabling noImplicitAny. It's possible this is incorrect.
    // TODO: @ENG-4160 Clean these up! If you're already touching this code, please clean this up while you're at it.
    .map((key) => mapV2.objects?.[key]);

  const collisions = collisionsMatrixToString(mapV2.collisions);

  return { ...mapV2, objects: objArray, collisions };
}

// soon hopefully we can box all this nonsense into just the game server, once we migrate old stuff away from old types
export function collisionsMatrixToString(collisions: boolean[][]): string {
  const collBytes: number[] = [];
  for (let r = 0; r < collisions.length; r++) {
    // @ts-expect-error Error auto-ignored when enabling TS noUncheckedIndexedAccess. If you're already touching this code, please clean this up while you're at it.
    // TODO: @ENG-4257 Clean these up! See the linear task for more context and advice for cleaning up.
    for (let c = 0; c < collisions[0].length; c++) {
      // @ts-expect-error Error auto-ignored when enabling TS noUncheckedIndexedAccess. If you're already touching this code, please clean this up while you're at it.
      // TODO: @ENG-4257 Clean these up! See the linear task for more context and advice for cleaning up.
      collBytes.push(collisions[r][c] ? 0x01 : 0x00);
    }
  }
  return new Buffer(collBytes).toString("base64"); // base64 encoded array of dimensions[1] x dimensions[0] bytes
}
