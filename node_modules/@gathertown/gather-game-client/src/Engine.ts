...
export class Engine {
  ...
  private async createConnection() {
    ...
    try {
      ...
      logger.error(
        `Failed to create websocket connection to gameserver ${this.serverURL} : ${logger.errString(
          e,
        )}`,
      );
      this.cleanUpAndQueueReconnect();
    ...
    logger.checkpoint(`t: ${Date.now() - this.connectionInitTime} : created ws connection`);
    ...
    this.ws.onopen = () => {
      ...
      if (this.disconnectedStartTime) {
        // if we've disconnected at least once, track for how long
      ...
      switch (event.$case) {
        ...
        case "ready":
          ...
          if (this.wantsToEnter) {
            // need to re-enter before doing anything else
  ...
  async getGameServerUrl() {
    ...
    if (!this.preferredRegion) {
      try {
        const res = await findClosestRegion();
        if (res) {
          logger.checkpoint(`t: ${Date.now() - this.connectionInitTime} : found closestRegion`);
          this.preferredRegion = res.closestRegion;
          this.preferredRegionPing = res.ping;
        }
      } catch (e) {
        logger.error("error finding closestRegion: " + logger.errString(e));
      }
    }

    const serverURL = await fetchGameServerAssignment(this.spaceId, this.preferredRegion);
